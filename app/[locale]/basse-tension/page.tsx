...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "BasseTensionPage" });
  return {
    title: t("metaTitle"),
...
export default async function BasseTensionPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("BasseTensionPage");
